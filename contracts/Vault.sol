...
contract Vault {
    // slot 0
    uint public count = 123;                        // Full 32 bytes
    // slot 1
    address public owner = msg.sender;              // Just 20 bytes, 12 bytes left
    bool public isTrue = true;                      // Just 1 byte, 11 bytes left
    uint16 public u16 = 31;                         // Just 2 bytes, 9 bytes LEFT
    // slot 2
    bytes32 private password;                       // Full 32 bytes

    // constants do not use storage
    uint public constant someConst = 123;

    // slot 3, 4, 5 (one for each array element, this is for defined arrays)
    ...
    // Assuming users.legth = 2, we have:
    // starting from slot: hash(6)     -> slot for uint id;     if: hash(6) + 1 -> slot for bytes32 password;   This is for index: 0
    //                 if: hash(6) + 2 -> slot for uint id;     if: hash(6) + 3 -> slot for bytes32 password;   This is for index: 1
